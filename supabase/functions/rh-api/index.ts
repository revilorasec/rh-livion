...
Deno.serve(async(req)=>{const origin=req.headers.get('origin');if(req.method==='OPTIONS')return new Response(null,{status:204,headers:cors(origin)});try{const url=new URL(req.url);const base='/rh-api';const i=url.pathname.indexOf(base);const path=i>=0?url.pathname.slice(i+base.length)||'/':'/';if(path==='/health'&&req.method==='GET')return reply(origin,200,{ok:true,service:'rh-api',version:2});const ctx=await rhAccess(req);if(path==='/data'&&req.method==='GET'){if(!has(ctx.actions,'rh.visualizar_dados_basicos')&&!ctx.admin)throw new Error('FORBIDDEN');const q=await db.from('portal_rh_data').select('payload,revision,updated_at,updated_by').eq('id',1).maybeSingle();if(q.error)throw q.error;const row=q.data||{payload:{},revision:0,updated_at:null,updated_by:null};const empty=!row.payload||Object.keys(row.payload).length===0;return reply(origin,200,{ok:true,empty,revision:row.revision,updatedAt:row.updated_at,updatedBy:row.updated_by,data:empty?{}:sanitize(row.payload,ctx),userType:ctx.userType,user:ctx.me,permissions:{salary:has(ctx.actions,'rh.visualizar_salarios'),bank:has(ctx.actions,'rh.visualizar_dados_bancarios'),sensitive:has(ctx.actions,'rh.visualizar_dados_sensiveis'),documents:ctx.userType==='INTERNO'&&has(ctx.actions,'rh.visualizar_documentos'),edit:has(ctx.actions,'rh.editar_funcionarios')}});}
